...
const SearchField = ({ value, placeholder, onChange }: SearchFieldProps) => (
  <div className="relative flex-1">
    <div className="inline-flex items-center w-full gap-2 relative py-0 px-4 h-12 min-h-12 border-2 border-border rounded-xs bg-background text-foreground focus-within:border-pink focus-within:outline-0 focus-within:outline-pink focus-within:outline-offset-0 [&>.icon]:text-primary">
      <GoSearch className="size-5 shrink-0 stroke-[0.3]" />
      <input
        value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="font-[inherit] py-3 px-4 text-base leading-snug text-foreground border border-border rounded block w-full placeholder:text-muted-foreground focus:outline-2 focus:outline-pink focus:outline-offset-0 disabled:cursor-not-allowed disabled:opacity-30 border-none flex-1 bg-transparent shadow-none outline-none -mx-4 max-w-none cursor-text!"
      />
      <div
        role="button"
        data-show={!!value}
